...
{
    public class CategoryRepository : GenericRepostory<Category>, ICategoryRepository
    {
        private readonly DbPosContext _context;

        public CategoryRepository(DbPosContext context)
        {
            _context = context;
        }
        ...
        {
            var categories = await _context.Categories.AsNoTracking().FirstOrDefaultAsync(x => x.CategoryId.Equals(categoryId));
            return categories!;
        }

        public async Task<BaseEntitiyResponse<Category>> ListCategories(BaseFilterRequest filters)
        {
            var response = new BaseEntitiyResponse<Category>();
